...
public class StepImplementation {
    ...
    public StepImplementation() {
        if(Driver.webDriver != null
            //!Driver.TestFileName.startsWith("/specs/ApiMigrosNext")
         ){
            methods = new Methods();
        }
    ...
    @Step("Get Current Url <mapKey>")
    public void getCurrentUrl(String mapKey){

        String url = methods.getCurrentUrl();
        Driver.TestMap.put(mapKey, url);
    }
    ...
    @Step("<key> elementi <condition> durumu true ise tıkla")
    public void clickElementIfExist(String key, String condition) {
    ...
    public void sendKeysFileUpload(String key, String fileUpload, boolean multipleFileActive, boolean customFileLocation) {
        ...
        if (multipleFileActive) {
            ...
            multipleFile.append(customFileLocation ? files.get(0) : System.getProperty("user.dir") + files.get(0));
            for (int i = 1; i < files.size(); i++) {
                multipleFile.append(" \n ").append(customFileLocation ? files.get(i) : System.getProperty("user.dir") + files.get(i));
            }
        }else {
            multipleFile.append(customFileLocation ? fileUpload : System.getProperty("user.dir") + fileUpload);
        }
    ...
    @Step("Şu anki url <url> ile <condition> durumunu sağlıyor mu <timeout>")
    public void doesUrl(String url, String condition, int timeout) {

        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url, 4*timeout, condition),"Beklenen url, sayfa url ine eşit değil");
    }
    ...
    public void doesUrlEqual(String url) {
        ...
        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url,120,"equal"),"Beklenen url, sayfa url ine eşit değil");
    }
    ...
    public void doesUrlContain(String url) {
        ...
        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url,120,"contain"),"Sayfa url, beklenen url i içermiyor");
    }
    ...
    public void doesUrlStartWith(String url) {
        ...
        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url,120,"startWith"),"Sayfa url, beklenen url ile başlamıyor");
    }
    ...
    public void doesUrlEndWith(String url) {
        ...
        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url,120,"endWith"),"Sayfa url, beklenen url ile sonlanmıyor");
    }
    ...
    public void checkUrlDifferent(String url) {
        ...
        url = url.endsWith("KeyValue") ? Driver.TestMap.get(url).toString() : url;
        url = methodsUtil.setValueWithMap(url);
        assertTrue(methods.doesUrl(url,120,"notEqual"),url + " sayfasından başka bir sayfaya geçiş sağlanamadı.");
    }
    ...
    public void checkElementEnabled(String key) {
        ...
        assertTrue(methods.isElementEnabled(methods.getBy(key),30),"");
    }

    @Step("<key> elementinin <isElementEnabled> enable degeri kontrol edilir <condition>")
    public void checkElementEnabledOrDisabled(String key, String isElementEnabled, String condition) {

        if(Boolean.parseBoolean(methodsUtil.getTextByMap(condition))) {
            isElementEnabled = methodsUtil.getTextByMap(isElementEnabled);
            assertEquals(Boolean.parseBoolean(isElementEnabled), methods.isElementEnabled(methods.getBy(key)));
        }
    }

    @Step("<key> elementinin var olduğu kontrol edilir")
    public void checkElementExist(String key) {
        assertTrue(methods.doesElementExist(methods.getBy(key),30),"");
    }

    @Step("<key> elementinin var olmadığı kontrol edilir")
    public void checkElementNotExist(String key) {

        assertTrue(methods.doesElementNotExist(methods.getBy(key),30),"");
    }
    ...
    @Step("<key> elementinin text değeriyle <expectedValue> değerini <condition> durumu kontrol edilir <timeout> trim <trim>")
    public void getElementText(String key, String expectedValue, String condition, int timeout, boolean trim){
        ...
        for (int i = 0; i < 4*timeout; i++) {
            value = methods.getText(by);
            value = trim ? value.trim() : value;
            if(methodsUtil.conditionValueControl(expectedValue, value, condition)){
        ...
        fail(value + " elementin değeriyle " + expectedValue + " eslesmedi");
    }
    ...
    public void getElementValueAndSave(String key, String mapKey, boolean trim){

        String value = methods.getValueJs(methods.getBy(key),"3");
        value = trim ? value.trim() : value;
        logger.info(value);
        Driver.TestMap.put(mapKey, value);
    }

    @Step("<key> elementinin value değeriyle <expectedValue> değerini <condition> durumu kontrol edilir <timeout> trim <trim>")
    public void getElementValue(String key, String expectedValue, String condition, int timeout, boolean trim){

        String value = "";
        expectedValue = expectedValue.endsWith("KeyValue") ? Driver.TestMap.get(expectedValue).toString() : expectedValue;
        By by = methods.getBy(key);
        for (int i = 0; i < 4*timeout; i++) {
            value = methods.getValueJs(by,"3");
            value = trim ? value.trim() : value;
            if(methodsUtil.conditionValueControl(expectedValue, value, condition)){
                System.out.println("expectedValue: " + expectedValue);
                System.out.println("actualValue: " + value);
        ...
        fail(expectedValue + " elementin değeriyle " + value + " eslesmedi");
    }
    ...
    public void checkElementAttributeNotContains(String key, String attribute, String expectedValue) {
        ...
        assertFalse(attributeValue.contains(expectedValue)
                ,expectedValue + " elementin değeriyle " + attributeValue + " eslesti");
    }
    ...
    @Step("<key> elementinin <attribute> niteliği <expectedValue> değerini <condition> durumu kontrol edilir <timeout>")
    public void checkElementAttribute(String key, String attribute, String expectedValue, String condition, int timeout) {

        expectedValue = expectedValue.endsWith("KeyValue") ? Driver.TestMap.get(expectedValue).toString() : expectedValue;
        String attributeValue = "";
        By by = methods.getBy(key);
        for (int i = 0; i < 4*timeout; i++) {
            attributeValue = methods.getAttribute(by, attribute);
            if(methodsUtil.conditionValueControl(expectedValue, attributeValue, condition)){
                System.out.println("expectedValue: " + expectedValue);
                System.out.println("actualValue: " + attributeValue);
                return;
            }
        ...
        fail(expectedValue + " elementin değeriyle " + attributeValue + " eslesmedi");
    }
    ...
    public void runConsoleJavaScript(String js, String arguments, String mapKey){
        ...
        js = js.endsWith("KeyValue") ? Driver.TestMap.get(js).toString() : js;
        logger.info(js);
        js = String.format(js, arguments.split("\\|\\?"));
        logger.info(js);
        Object value = methods.jsExecuteScript(js,false);
        //logger.info(value.toString());
        Driver.TestMap.put(mapKey, value);
